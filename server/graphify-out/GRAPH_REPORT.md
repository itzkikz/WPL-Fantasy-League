...
## Corpus Check
- 72 files · ~71,530 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 429 nodes · 1020 edges · 15 communities (14 shown, 1 thin omitted)
...
- `dashboard()` --indirect_call--> `error`  [INFERRED]
  src/controllers/manager.ts → src/types/error.ts
- `getPlayerStats()` --indirect_call--> `error`  [INFERRED]
  src/controllers/players.ts → src/types/error.ts
...
Cohesion: 0.11
Nodes (50): completeGameweek(), createFantasyTeam(), createGameweek(), createH2HFixture(), deleteH2HFixture(), deleteH2HLeague(), fetchLeagueRounds(), getAdminPlayers() (+42 more)
...
Cohesion: 0.10
Nodes (39): dashboard(), details(), myFixtures(), substitution(), aggregateMatchStats(), getFilters(), getFullPlayerStats(), getPlayerStats() (+31 more)

### Community 3 - "index.ts"
Cohesion: 0.07
Nodes (27): getFixtures(), SheetController, app, auth, cors, credentials, fs, { google } (+19 more)
...
Cohesion: 0.12
Nodes (17): client, privateKey, Cell, notifications(), Row, send(), subscribe(), webpush (+9 more)
...
- **Why does `error` connect `admin.ts` to `seedFixtures.ts`, `manager.ts`, `index.ts`, `transfers.ts`, `fetchSofascoreData.js`?**
  _High betweenness centrality (0.099) - this node is a cross-community bridge._
- **Why does `main()` connect `fetchSofascoreData.js` to `admin.ts`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Why does `FantasyTeam` connect `admin.ts` to `notification.ts`, `transfers.ts`, `manager.ts`, `FantasyTeam.ts`?**
