...
const bucket = process.env.SUPABASE_BUCKET || 'player-images';
...
const DELAY_MS = 1500;
const SOFASCORE_IMAGE_BASE = 'https://img.sofascore.com/api/v1/player';
...
const uploadPlayerImages = async () => {
    try {
        await connectDB();
        ...
        if (players.length === 0) {
            console.warn('No players found. Run seed:players first.');
            process.exit(0);
        }
        ...
        while (true) {
            const { data, error: listError } = await supabase.storage
                .from(bucket)
                .list('players', { limit: pageLimit, offset });
            if (listError) {
        ...
        try {
            for (const [index, player] of players.entries()) {
                ...
                const playerName = p.name || `Player ${playerId}`;

                if (existingImages.has(playerId)) {
                    const ext = existingImages.get(playerId);
                    const photoUrl = `${publicBaseUrl}/players/${playerId}.${ext}`;
                    await Player.updateOne({ id: playerId }, { $set: { photo: photoUrl } });
                    alreadyExists++;
                    continue;
                }

                try {
                    const imageUrl = `${SOFASCORE_IMAGE_BASE}/${playerId}/image`;
                    ...
                    if ('error' in result) {
                        console.warn(`  [${index + 1}/${players.length}] ${playerName} (${playerId}) - ${result.error}`);
                        skipped++;
                        continue;
                    }

                    if (!result.base64 || result.size === 0) {
                        console.warn(`  [${index + 1}/${players.length}] ${playerName} (${playerId}) - Empty image`);
                        skipped++;
                        continue;
                    }
                    ...
                    if (!detectedType) {
                        ...
                        console.warn(`  [${index + 1}/${players.length}] ${playerName} (${playerId}) - Not a valid image (${result.contentType}, ${result.size} bytes). First bytes: ${preview}`);
                        skipped++;
                        continue;
                    }
                    ...
                    if (error) {
                        console.error(`  [${index + 1}/${players.length}] ${playerName} (${playerId}) - Upload error: ${error.message} (${detectedType}, ${result.size} bytes)`);
                        errors++;
                        continue;
                    }

                    const photoUrl = `${publicBaseUrl}/${filePath}`;
                    await Player.updateOne({ id: playerId }, { $set: { photo: photoUrl } });

                    uploaded++;
                    if ((index + 1) % 50 === 0 || index === players.length - 1) {
                        console.log(`  Progress: ${index + 1}/${players.length} (${uploaded} uploaded, ${alreadyExists} existing, ${skipped} skipped, ${errors} errors)`);
                    }
                } catch (err: any) {
                    console.error(`  [${index + 1}/${players.length}] ${playerName} (${playerId}) - ${err.message}`);
                    errors++;
                }
        ...
        console.log(`\nDone. ${uploaded} uploaded, ${alreadyExists} already existed, ${skipped} skipped, ${errors} errors out of ${players.length} players.`);
        process.exit(0);
    } catch (error) {
