      ...
      <div className="bg-[#1b142d]/80 border border-white/10 p-4 sm:p-5 rounded-2xl shadow-xl space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-white/10 pb-3.5">
          <div className="space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-sm font-black uppercase tracking-wider text-white flex items-center gap-1.5">
                ...
                Squad Lock & Transfer Deadline
              </h2>
              <span
                className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-full border ${
                  !pickTeamData?.data?.enabled
                    ? "bg-rose-500/10 text-rose-400 border-rose-500/30"
                    : pickTeamData?.data?.deadlineDate && new Date() > new Date(pickTeamData.data.deadlineDate)
            ...
            <p className="text-xs text-white/60 leading-relaxed">
              Squad transfers automatically lock when the deadline timestamp passes or when manually locked.
            </p>
          </div>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-black transition-all shadow-md flex items-center justify-center gap-1.5 active:scale-95 border cursor-pointer ${
              pickTeamData?.data?.enabled
          ...
          >
            {togglePickTeamMutation.isPending ? (
              ...
              <>
                <Lock className="w-4 h-4 text-rose-400" /> Lock Squad Selection
              </>
            ) : (
              <>
                <Unlock className="w-4 h-4 text-emerald-400" /> Unlock Squad Selection
              </>
            )}
          </button>
        </div>

        {/* Deadline Input & Action Row */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-[#150f24]/70 border border-white/5 p-3 rounded-xl">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {gameweeks.find((gw: any) => gw.isCurrent) && (
              <button
                type="button"
                onClick={() => {
                ...
                disabled={togglePickTeamMutation.isPending}
                className="px-3 py-1.5 rounded-lg text-xs font-bold bg-indigo-500/10 text-indigo-300 border border-indigo-500/25 hover:bg-indigo-500/20 transition-all cursor-pointer"
                title="Sync deadline with the start date of the active Gameweek"
              >
                Sync GW Start
              </button>
            )}
            ...
            >
              {togglePickTeamMutation.isPending ? (
                ...
                <>
                  <Save className="w-3.5 h-3.5" /> Save Deadline
                </>
              )}
            </button>
          </div>
