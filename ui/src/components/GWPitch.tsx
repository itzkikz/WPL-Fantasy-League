...
const GWPitch = ({
  ...
  console.log("test", isSubstitution, starting, bench);
  return (
    <>
      <div
        ...
        {Object.keys(starting).map((key: string) => (
          <div key={key} className="flex justify-evenly gap-8">
            {starting[key].map((player: Player) => (
              <div
                key={player.id}
                className={`flex flex-col ${!player?.isAvlSub && isSubstitution ? "opacity-40" : ""}`}
              >
        ...
        <div className="relative w-auto bottom-0 left-0 right-0 bg-gradient-to-b from-green-800/80 to-green-900/90 backdrop-blur-sm py-2 px-2 mt-8">
          ...
          <div className="flex justify-center gap-6 md:gap-8">
            {bench?.map((player) => (
              <div
                key={player.id}
                className={`${!player?.isAvlSub && isSubstitution ? "opacity-40" : ""}`}
              >
                <PitchPlayerCard
                  player={player}
                  isSmall={true}
